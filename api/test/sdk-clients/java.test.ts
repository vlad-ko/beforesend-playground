import { transformWithJava } from '../../src/sdk-clients/java';
import axios from 'axios';

jest.mock('axios');
const mockedAxios = axios as jest.Mocked<typeof axios>;

describe('Java SDK Client', () => {
  beforeEach(() => {
    mockedAxios.post.mockClear();
  });

  it('should successfully transform event with Java SDK', async () => {
    const event = {
      exception: {
        values: [
          {
    ...
    const result = await transformWithJava(event, beforeSendCode);

    expect(result.success).toBe(true);
    expect(result.transformedEvent?.tags?.transformed).toBe('true');
    expect(mockedAxios.post).toHaveBeenCalledWith(
      'http://sdk-java:5007/transform',
      {
        event,
        beforeSendCode,
      },
  ...
  it('should handle SDK service errors', async () => {
    const event = { message: 'test' };
    const beforeSendCode = 'invalid java code';
  ...
  it('should handle connection errors to SDK service', async () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error).toContain('Failed to connect to Java SDK service');
  });
