import { transformWithDotNet } from '../../src/sdk-clients/dotnet';
import axios from 'axios';

jest.mock('axios');
const mockedAxios = axios as jest.Mocked<typeof axios>;

describe('.NET SDK Client', () => {
  beforeEach(() => {
    mockedAxios.post.mockClear();
  });

  it('should successfully transform event with .NET SDK', async () => {
    const event = {
      exception: {
        values: [
          {
    ...
    const result = await transformWithDotNet(event, beforeSendCode);

    expect(result.success).toBe(true);
    expect(result.transformedEvent?.Tags?.transformed).toBe('true');
    expect(mockedAxios.post).toHaveBeenCalledWith(
      'http://sdk-dotnet:5002/transform',
      {
        event,
        beforeSendCode,
      },
  ...
  it('should handle SDK service errors', async () => {
    const event = { message: 'test' };
    const beforeSendCode = 'invalid csharp code';
  ...
  it('should handle connection errors to SDK service', async () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error).toContain('Failed to connect to .NET SDK service');
  });
